...
class FileStream(object):
    ...
    def read_from_blast(self, filename):
        ...
        """
        blast_file_path = (
            ReplicladeSettings.DNA_PATH + "{}.xml".format(filename)
        )
        blast_seqs = []
    ...
    def write_to_fasta_sim_results(sequences, nodes, filename_original):
        ...
        with open(ReplicladeSettings.DNA_PATH + "{}.fasta".format(filename), "w") as open_file:
    ...
    def write_to_fasta_results(seq_id, sequences):
        with open(
            ReplicladeSettings.RESULTS_PATH + "{}_results.fasta".format(seq_id.replace("|", "")), "w"
        ) as open_file:
    ...
    def write_to_fasta_results_multiple(seq_ids, sequences):
        with open(ReplicladeSettings.RESULTS_PATH + "post_sim_results.fasta", "w") as open_file:
            for i in range(len(sequences)):
    ...
    def log_simulation_to_json(generation_dict):
        ...
        print("Results written to {}".format(ReplicladeSettings.RESULTS_PATH + filename))
