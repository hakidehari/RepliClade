...
GENE_NAME_LIST = ["cytochrome_b", "cytochrome_b_1"]
...
class Simulator(object):
    ...
    def gene_prompt(self):
        ...
        while gene_choice not in GENE_NAME_LIST:
            gene_choice = input(
                "Invalid choice.  Please choose one of the genes listed above: "
            )
        self.gene = gene_choice

    def file_prompt(self):
        file_prompt = input(
            "Would you like to use an input file?  Please specify Y or N:  "
        )
    ...
    def prompt_motif_alg(self):
        ...
        MOTIFS = ["deterministic", "probabilistic"]

        DET_ALGS = ["exhaustive", "branch_and_bound", "heuristic"]

        PROB_ALGS = ["heuristic_stochastic", "gibbs"]

        print(
            "Now that the sequences have been aligned, which off the following motif finding \n \
        ...
        while choice.lower() not in MOTIFS:
            print(
                "Invalid input.  Please specify one of the motif finding algorithms provided"
            )
            for alg in MOTIFS:
                print(alg)
            choice = input("-->")
        ...
        if self.motif_alg == "deterministic":
            ...
            while choice.lower() not in DET_ALGS:
                print(
                    "Invalid input.  Please specify one of the deterministic motif finding algorithms provided"
                )
                for alg in DET_ALGS:
                    print(alg)
                choice = input("-->")
        ...
        if self.motif_alg == "probabilistic":
            ...
            while choice.lower() not in PROB_ALGS:
                print(
                    "Invalid input.  Please specift one of the probabilistic motif finding algorithms provided"
                )
                for alg in PROB_ALGS:
                    print(alg)
                choice = input("-->")
    ...
    def simulate(self, c_regions, filename):
        ...
        if align_results.lower() == "y":
            ...
            if single_or_multiple == "single":
                ...
                seq_util.align_results_w2_single(
                    generation_dict, generations, index, seq_id
                )
            if single_or_multiple == "multiple":
                seq_util.align_results_w2_multiple(
                    generation_dict, generations, seq_ids
                )
    ...
    def thesis_results_sim(self, ancestral_seq, entropy_scores, filename):
        ...
        for model in models:
            for i in range(54, 100):
                ...
                tree, post_sim_seqs, true_tree = self.simulate_ancestor(
                    ancestral_seq, seq_util.mu, entropy_scores, model
                )
                ...
                Phylo.write(
                    ...
                    + os.path.sep
                    + filename
                    + "-trees_{0}_{1}.xml".format(model, i),
                    "phyloxml",
                )
                trees = []
