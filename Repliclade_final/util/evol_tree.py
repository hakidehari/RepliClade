...
class Phylogenize(object):
            ...
            for j in range(len(self.__sequences)):
                if i == j:
                    ...
                    for k in range(len(seq1)):
                        ...
                        char2 = seq2[k]
                        if (
                            char2 in transversions[char1]
                            or char1 in transversions[char2]
                        ):
                            q += 1
                    ...
                    tp = float(p / len(seq1))
                    tv = float(q / len(seq1))
                    k = -0.5 * np.log(1 - 2 * tp - tv) - 0.25 * np.log(1 - 2 * tv)
                    dist_matrix[i][j] = k
    ...
    def prompt_tree_builder(self):
        ...
        method = input(
            "You can select nj=neighbor joining, upgma=UPGMA, parsimony=Maximum Parsimony, ml=Maximum Likelihood: "
        )
        while method not in choices:
            method = input(
                "Invalid choice, please choose one of the options above (nj, upgma, parsimony, or ml): "
            )
        return method
