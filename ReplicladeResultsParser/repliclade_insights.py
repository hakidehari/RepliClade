...
def calculate_tree_similarity(trees):
    ...
    for file in trees:
        ...
        for i in range(4):
            other_tree = file[i]
            true_terminals = [
                clade.name for clade in TreeMixin.get_terminals(true_tree)
            ]
            other_terminals = [
                clade.name
                for clade in TreeMixin.get_terminals(other_tree)
                if clade.name is not None and "Inner" not in clade.name
            ]
            if (
                TreeMixin.get_nonterminals(true_tree) is not None
                and TreeMixin.get_nonterminals(other_tree) is not None
            ):
                true_nonterminals = [
                    clade.name for clade in TreeMixin.get_nonterminals(true_tree)
                ]
                other_nonterminals = [
                    clade.name
                    for clade in TreeMixin.get_nonterminals(other_tree)
                    if clade.name is not None and "Inner" not in clade.name
                ]
            else:
                true_nonterminals = []
            ...
            terminal_nonterminal_score = compare_terminals(
                true_terminals,
                other_terminals,
                true_nonterminals,
                other_nonterminals,
...
def compare_terminals(
    ...
    total_terminals = (
        len(true_terminals) + len(other_terminals) - len_intersecting_terminals
    )
    ...
    len_intersecting_nonterminals = len(
        true_nonterminals.intersection(other_nonterminals)
    )
    total_nonterminals = (
        len(true_nonterminals) + len(other_nonterminals) - len_intersecting_nonterminals
    )
    ...
    preterminal_score = (
        1
        if preterminal_count_true == preterminal_count_other
        else abs(preterminal_count_other - preterminal_count_true)
    )
